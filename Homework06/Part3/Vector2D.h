...
class Vector2D{
    ...
    public:
        Vector2D(int, int, T);// constructor with default values 
        ~Vector2D();//destructor to get rid of the memory 
        Vector2D(const Vector2D&); // copy constructor
        Vector2D operator=(const Vector2D&); // overload assignment for this class
        void display();
        ...
        template<class U>
        friend ostream& operator<<(ostream &, const Vector2D<U>&); // overload out stream
        // overload access operators
        vector<T>& operator[](int); // overload [] array accessor
        bool operator==(const Vector2D&);// overload equals relational operator ==
        bool operator!=(const Vector2D&); // overload does not equal operator
...
*/
template<class T>
Vector2D<T> Vector2D<T>::operator=(const Vector2D &right)
{
    // init the rows and cols
    rows = right.rows; 
    cols = right.cols;
...
*/
template<class T>
void Vector2D<T>::display(int spaceX)
{
    for(int i = 0; i<rows; i++)
    {
...
*/
template<class T>
void Vector2D<T>::set(int r, int c, T var)
{
...
*/
template<class T>
void Vector2D<T>::resize(int r, int c)
{
...
*/
template<class T>
ostream& operator<<(ostream &strm, const Vector2D<T> &val)
{
    strm<<"["; 
...
*/
template<class T>
vector<T>& Vector2D<T>::operator[](int n)
{
